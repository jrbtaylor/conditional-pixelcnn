...
def run(batch_size=64,n_features=200,n_layers=6,n_bins=16,
    ...
    exp_name += '_%ifeat_%ilayers_%ibins'%(
        n_features,n_layers,n_bins)
    ...
    def onehot(x):
        def _onehot(x):
            ...
            y[0,x] = 1
            return y
        return torch.from_numpy(np.concatenate([_onehot(v) for v in x]))
    loss_fcn = torch.nn.NLLLoss2d(torch.autograd.Variable(weight))

    # Train
    train.fit(train_loader,val_loader,net,exp_dir,input2label,onehot,loss_fcn,
